...
interface ExternalAlzheimerGameProps {
  isVisible: boolean;
  onClose: () => void;
  onGameResult: (result: { mmse: number; cdr: number; digital: number }) => void;
}

const GAME_ORIGIN = 'http://127.0.0.1:8001';
const GAME_URL = `${GAME_ORIGIN}/alzheimer-game/`;

export const ExternalAlzheimerGame: React.FC<ExternalAlzheimerGameProps> = ({
  isVisible,
  onClose,
  onGameResult,
}) => {
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      const data = event.data as Partial<{ type: string; mmse: number; cdr: number; digital: number }>;
      if (data?.type === 'alzheimer-game-finished' && typeof data.mmse === 'number' && typeof data.cdr === 'number' && typeof data.digital === 'number') {
        onGameResult({ mmse: data.mmse, cdr: data.cdr, digital: data.digital });
      }
    };
        ...
        >
          <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3 text-white md:px-6">
            <div>
              ...
              <h3 className="text-lg font-semibold">Alzheimer Game from ML Folder</h3>
            </div>
