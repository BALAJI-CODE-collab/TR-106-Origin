...
interface ExternalSpeechTherapyGameProps {
  isVisible: boolean;
  onClose: () => void;
  onGameResult: (result: { clarity: number; pace: number; stability: number }) => void;
}

const GAME_ORIGIN = 'http://127.0.0.1:8001';
const GAME_URL = `${GAME_ORIGIN}/speech-therapy-game/`;

export const ExternalSpeechTherapyGame: React.FC<ExternalSpeechTherapyGameProps> = ({
  isVisible,
  onClose,
  onGameResult,
}) => {
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (
        data?.type === 'speech-therapy-game-finished' &&
      ...
      >
        ...
        >
          <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3 text-white md:px-6">
            <div>
              <p className="text-xs uppercase tracking-[0.28em] text-emerald-200/70">External ML Project</p>
              <h3 className="text-lg font-semibold">Speech Therapy Game from ML Folder</h3>
            </div>
